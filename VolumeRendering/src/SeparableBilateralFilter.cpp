...
vtkStandardNewMacro(SeparableBilateralFilter);
...
//----------------------------------------------------------------------------
void SeparableBilateralFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void SeparableBilateralFilter::ComputeKernel(double *kernel, int min, int max)
{
  int x;
  double sum;
  ...
  {
    sum += kernel[x - min] =
      exp(-(static_cast<double>(x*x)) / (Sigma * Sigma * 2.0));
  }
...
//----------------------------------------------------------------------------
int SeparableBilateralFilter::RequestUpdateExtent(
...
//----------------------------------------------------------------------------
void SeparableBilateralFilter::InternalRequestUpdateExtent(int *inExt,
  int *wholeExtent)
{
  int idx, radius;
...
template <class T>
void
SeparableBilateralFilterExecute(SeparableBilateralFilter *self, int axis,
  ...
  for (idxC = 0; idxC < maxC; ++idxC)
  {
    ...
    for (idx1 = 0; !self->AbortExecute && idx1 < max1; ++idx1)
    {
      ...
      for (idx0 = 0; idx0 < max0; ++idx0)
      {
        ...
        sum = 0.0;
        tW = 0.0;
        iC = static_cast<double>(*(inPtrK + kernelMiddle));
        // too bad this short loop has to be the inner most loop
        for (idxK = 0; idxK < kernelSize; ++idxK)
        {
          v = static_cast<double>(*inPtrK);
          tW += w = *ptrK * exp(-((abs(v - iC)) / Sigmas2));
          sum += w*v;
          ++ptrK;
          inPtrK += inIncK;
        }
        *outPtr0 = static_cast<T>(sum/tW);
        inPtr0 += inInc0;
        outPtr0 += outInc0;
      }
      inPtr1 += inInc1;
...
//----------------------------------------------------------------------------
template <class T>
size_t SeparableBilateralFilterGetTypeSize(T*)
{
  return sizeof(T);
}
...
void SeparableBilateralFilter::ThreadedRequestData(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector,
  vtkImageData ***inData,
...
double SeparableBilateralFilter::ItlComputeSigma(int _KernelSize)
{
  return 0.3*((_KernelSize - 1)*0.5 - 1) + 0.8;
}

void SeparableBilateralFilter::SetKernelSize(int _KernelSize)
{
