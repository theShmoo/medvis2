...
vtkStandardNewMacro(GaussianFilter);
...
//----------------------------------------------------------------------------
void GaussianFilter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void GaussianFilter::ComputeKernel(double *kernel, int min, int max)
{
  int x;
  double sum;
...
//----------------------------------------------------------------------------
int GaussianFilter::RequestUpdateExtent (
...
//----------------------------------------------------------------------------
void GaussianFilter::InternalRequestUpdateExtent(int *inExt,
                                                         int *wholeExtent)
{
  int idx, radius;
...
template <class T>
void
GaussianFilterExecute(GaussianFilter *self, int axis,
...
//----------------------------------------------------------------------------
template <class T>
size_t GaussianFilterGetTypeSize(T*)
{
  return sizeof(T);
}
...
void GaussianFilter::ExecuteAxis(int axis,
                                         ...
                                         vtkInformation *inInfo)
{
  ...
  switch (outData->GetScalarType())
    {
    vtkTemplateMacro(
      outIncA *= GaussianFilterGetTypeSize(static_cast<VTK_TT*>(0))
      );
    default:
  ...
  for (idxA = outExt[axis*2]; idxA <= max; ++idxA)
    ...
    switch (inData->GetScalarType())
      {
      vtkTemplateMacro(
        GaussianFilterExecute(this, axis, kernel, kernelSizeClipped,
...
void GaussianFilter::ThreadedRequestData(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector,
  vtkImageData ***inData,
